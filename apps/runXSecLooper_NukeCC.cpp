...
bool debug=false;
bool anyTracker=true;

double GetNormValue( int targetID, int targetZ, int targetNucleon = 0 )
{

  //double trackerAtomsC = TargetUtils::Get().GetTrackerElementNAtoms( 6, 5990., 8340., 1 ); //z 5990-8340 = [27,78]=52 modules
...
public:
    ...
    double Qsquared(ChainWrapper& chw, int entry){
        ...
        const double M_muon2= pow(105.66, 2);
        ...
        double Q2_approxed = 4*neutrino_E*muon_E*pow( sin( muon_theta /2 ), 2. )/(1000*1000);
        //        cout<<"The Q2 is "<<Q2<<endl;
	double pmu2=pow(muon_E,2) - M_muon2;
	double pmu= (pmu2>0)? sqrt(pmu2):0;
	double Q2= 2* neutrino_E* (muon_E - pmu*cos(muon_theta)  ) - M_muon2;
	Q2/= 1000*1000;
    ...
    bool isDIS( ChainWrapper& chw, int entry ) {
        //        cout<<"I'm calling the signal definition "<<endl;
        const double PI = atan(1.0)*4;
        const double deg_to_rad = PI / 180.0;
        const double rad_to_deg = 1. / deg_to_rad;
	...
	//No longer included in signal definition
...
{
  public:
    NukeCC_XSecLooper(const char* inputFileGlob) :  XSecLooper(inputFileGlob){};

  //override this function thats cutting out target events, fiducial is defined in event selection.
  bool isFiducial(ChainWrapper& chw, int entry){
    //cout<<"runXSecLooper:isFiducial"<<endl;
    return true;
  }
...
void runXSecLooper_NukeCC( )
{
    string playlist="minervame1D";
...
//    output_name += "_" + target_type;
    ...
    //"/minerva/data/users/norrick/NukeTuples/v1_WithTargetCode_MLNX/NukeTruth_AnaTuple_minervame1A_v1_WithTargetCode_MLNX.root");
    //    XSecLooper loop("/minerva/data/users/norrick/NukeTuples/v1_WithTargetCode_MLNX/NukeTruth_AnaTuple_Tgt*minervame1D*.root");
    NukeCC_XSecLooper loop(Form("%s/%s/NukeCC_MC_AnaTuple_%s_*_%s.root", getenv("TUPLES"), getenv("NUKECC_V"), playlist.c_str(), getenv("NUKECC_V")) );

    //This is probably wrong. 
    //! Tell the XSecLooper which neutrino type we're considering (mandatory)
    loop.setNuPDG(14);
    //loop.setFiducial(true, 5990, 8340);//not the order of right arguments for this function anymore (theres no bool)
    loop.setFiducial(5980,8375);//modules [27,79]
...
/*   targetIDs.push_back(34);
    ...
    for(int i=0; i <targetIDs.size(); ++i){
        ...
        for(int j=0; j <targetZs.size(); ++j){
            ...
            for( vector<XSec::EVariable>::iterator var = vars.begin(); var != vars.end(); ++var ){
                ...
                char* name = Form( "%s_%d_%s_std_dis", nuc_name.c_str(), num_target, varName.c_str());
                ...
                xsec->setFluxIntLimits(0.,120.);//(0,50);//
                xsec->setUniverses(0);
		//xsec->setNormalizationType(XSec::kPerNucleon);
                xsec->setNormalizationType(XSec::kSelfNorm);
		xsec->setDimension(1);//Not sure why I suddenly need this? weird...
                //! add cross section
                loop.addXSec(xsec);
    ...
    string fname = "/minerva/data/users/${USER}/NukeFiles/NukeCC_xsec_minervame1D_targets_NuEOn.root";
...
//        string run_type         = "";
	...
	TH1::AddDirectory(false);//maybe I need this???
//        runXSecLooper_NukeCC( target_type );
            runXSecLooper_NukeCC();
