...
double GetNormValue( int targetID, int targetZ, int targetNucleon = 0 )
{
    ...
    double trackerAtomsC = TargetUtils::Get().GetTrackerElementNAtoms( 6, 104., 1 );
...
public:
    ...
    bool PassTrueDistToDivisionCut( ChainWrapper& chw, int entry, int targetID, int num_targetZ, double xySep /* = 25. */ )
    {
        ...
        if( targetID == 94 && ( true_module < 75 || true_module > 81)) return false;
...
//    XSecLooper loop("/minerva/data/users/norrick/NukeTuples/v1_WithTargetCode_MLNX/NukeTruth_AnaTuple_Tgt*minervame1D*.root");
    XSecLooper loop("/minerva/data/users/norrick/NukeTuples/v1_WithTargetCode_MLNX/NukeTruth_AnaTuple_minervame1A_v1_WithTargetCode_MLNX.root");
...
/*   targetIDs.push_back(34);
    ...
    string fname = "/minerva/data/users/${USER}/NukeFiles/NukeCC_xsec_minervame1A_Sept4.root";
